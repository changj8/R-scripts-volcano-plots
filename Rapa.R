...
df <- read.csv("Rapa-tryp-EMT.csv")

df$threshold = as.factor(df$p < 0.05)
df <- df[-1,]

tiff(filename='Rapa-tryp-EMT-volcano.tiff', width=7*150, height=5*150, res=150)

ggplot(data=df, aes(x=log2, y=nlogp, colour=threshold, label=Gene)) + geom_point(alpha=1, size=3) + 
  geom_text(data=subset(df, p < 0.05), nudge_x=0.12, nudge_y=-0.08, color='black', size=2) +
