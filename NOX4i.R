...
df <- read.csv("NOX4i-scratch-EMT.csv")

df$threshold = as.factor(df$p < 0.05)
df <- df[-1,]
df <- df[-11,]

tiff(filename='NOX4i-scratch-EMT-volcano.tiff', width=7*150, height=5*150, res=150)

ggplot(data=df, aes(x=log2, y=nlogp, colour=threshold, label=Gene)) + geom_point(alpha=1, size=3) + 
  geom_text(data=subset(df, p < 0.05), nudge_x=0.15, nudge_y=-0.06, color='black', size=2) +
